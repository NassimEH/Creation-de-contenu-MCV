/**
 * Générateur de contenu SEO pour les pages VTC des communes du Val-d'Oise (95).
...
import { VTC_VAL_DOISE_VILLES_A } from "./vtc-val-doise-villes-a.js";
import { VTC_VAL_DOISE_VILLES_B } from "./vtc-val-doise-villes-b.js";
import { VTC_VAL_DOISE_VILLES_C } from "./vtc-val-doise-villes-c.js";
import { VTC_VAL_DOISE_VILLES_D } from "./vtc-val-doise-villes-d.js";
import { VTC_VAL_DOISE_VILLES_E } from "./vtc-val-doise-villes-e.js";
import { VTC_VAL_DOISE_VILLES_F } from "./vtc-val-doise-villes-f.js";
import { VTC_VAL_DOISE_VILLES_G } from "./vtc-val-doise-villes-g.js";
import { VTC_VAL_DOISE_VILLES_H } from "./vtc-val-doise-villes-h.js";
import { VTC_VAL_DOISE_VILLES_I } from "./vtc-val-doise-villes-i.js";
import { VTC_VAL_DOISE_VILLES_J } from "./vtc-val-doise-villes-j.js";
import { VTC_VAL_DOISE_VILLES_L } from "./vtc-val-doise-villes-l.js";
import { VTC_VAL_DOISE_VILLES_M } from "./vtc-val-doise-villes-m.js";
import { VTC_VAL_DOISE_VILLES_N } from "./vtc-val-doise-villes-n.js";
import { VTC_VAL_DOISE_VILLES_O } from "./vtc-val-doise-villes-o.js";
import { VTC_VAL_DOISE_VILLES_P } from "./vtc-val-doise-villes-p.js";
import { VTC_VAL_DOISE_VILLES_R } from "./vtc-val-doise-villes-r.js";
import { VTC_VAL_DOISE_VILLES_S } from "./vtc-val-doise-villes-s.js";
import { VTC_VAL_DOISE_VILLES_T } from "./vtc-val-doise-villes-t.js";
import { VTC_VAL_DOISE_VILLES_U } from "./vtc-val-doise-villes-u.js";
import { VTC_VAL_DOISE_VILLES_V } from "./vtc-val-doise-villes-v.js";
import { VTC_VAL_DOISE_VILLES_W } from "./vtc-val-doise-villes-w.js";

const ALL_COMMUNES = [
  ...VTC_VAL_DOISE_VILLES_A,
  ...VTC_VAL_DOISE_VILLES_B,
  ...VTC_VAL_DOISE_VILLES_C,
  ...VTC_VAL_DOISE_VILLES_D,
  ...VTC_VAL_DOISE_VILLES_E,
  ...VTC_VAL_DOISE_VILLES_F,
  ...VTC_VAL_DOISE_VILLES_G,
  ...VTC_VAL_DOISE_VILLES_H,
  ...VTC_VAL_DOISE_VILLES_I,
  ...VTC_VAL_DOISE_VILLES_J,
  ...VTC_VAL_DOISE_VILLES_L,
  ...VTC_VAL_DOISE_VILLES_M,
  ...VTC_VAL_DOISE_VILLES_N,
  ...VTC_VAL_DOISE_VILLES_O,
  ...VTC_VAL_DOISE_VILLES_P,
  ...VTC_VAL_DOISE_VILLES_R,
  ...VTC_VAL_DOISE_VILLES_S,
  ...VTC_VAL_DOISE_VILLES_T,
  ...VTC_VAL_DOISE_VILLES_U,
  ...VTC_VAL_DOISE_VILLES_V,
  ...VTC_VAL_DOISE_VILLES_W,
];
...
function metaDescription(commune) {
  ...
  let text = `${kw} : chauffeur privé à ${commune.name} (${commune.cp || "95"}), transferts aéroports et gares. Réservation simple, 24h/24.`;
...
/** Gares à mentionner selon le canton (Val-d'Oise 95). */
function getGaresLabel(commune) {
  const c = (commune.canton || "").toLowerCase();
  if (c.includes("pontoise")) return "gares de Pontoise ou de Paris";
  if (c.includes("vauréal") || c.includes("vaureal")) return "gares de Vauréal ou de Paris";
  if (c.includes("montmorency")) return "gares de Montmorency ou de Paris";
  if (c.includes("argenteuil")) return "gares d'Argenteuil ou de Paris";
  if (c.includes("garges")) return "gares de Garges-lès-Gonesse ou de Paris";
  if (c.includes("isle-adam") || c.includes("l'isle")) return "gares de L'Isle-Adam ou de Paris";
  if (c.includes("fosses")) return "gares de Fosses ou de Paris";
  if (c.includes("saint-ouen-l'aumône") || c.includes("saint-ouen-l'aumone")) return "gares de Saint-Ouen-l'Aumône ou de Paris";
  if (c.includes("domont")) return "gares de Domont ou de Paris";
  if (c.includes("taverny")) return "gares de Taverny ou de Paris";
  if (c.includes("cergy")) return "gares de Cergy ou de Paris";
  if (c.includes("franconville")) return "gares de Franconville ou de Paris";
  if (c.includes("villiers-le-bel")) return "gares de Villiers-le-Bel ou de Paris";
  if (c.includes("herblay")) return "gares d'Herblay ou de Paris";
  if (c.includes("deuil-la-barre")) return "gares de Deuil-la-Barre ou de Paris";
  if (c.includes("ermont")) return "gares d'Ermont ou de Paris";
  if (c.includes("goussainville")) return "gares de Goussainville ou de Paris";
  if (c.includes("sarcelles")) return "gares de Sarcelles ou de Paris";
  return "gares de Paris ou de Cergy-Pontoise";
}
...
`;
  ...
  const IMAGE_URL = "https://monchauffeurvipv3.netopsia.eu/wp-content/uploads/2026/02/vtc-val-doise.webp";
...
<p>La ponctualité est au cœur de notre engagement : nous suivons le trafic en temps réel et adaptons les itinéraires pour respecter vos horaires, notamment pour les transferts aéroports ou gares. Que vous habitiez ou travailliez à ${locCourt}, dans le canton de ${district} ou aux environs, nous nous déplaçons à votre adresse. De plus, nous couvrons aussi les communes voisines du Val-d'Oise.</p>
...
`;
  ...
  const sectionCouverture = getSectionCouverture({ locCourt, cp, district, gares, kw, departmentName: "Val-d'Oise" });
  const sectionQuandReserver = getSectionQuandReserver({ locCourt, cp, kw, departmentName: "Val-d'Oise" });

  const sectionPourquoi = `
<h2>Pourquoi choisir notre ${kw}</h2>
<p>Choisir notre service, c'est opter pour un transport privé de qualité, adapté aux déplacements dans le Val-d'Oise et en Île-de-France. En effet, nous nous engageons sur la ponctualité et le confort ; de plus, notre offre répond aux besoins des particuliers comme des professionnels.</p>
...
<p>Le transfert vers l'aéroport Paris-Charles de Gaulle (Roissy) depuis ${locCourt} est l'un de nos trajets les plus réservés. La distance depuis le Val-d'Oise varie selon votre commune ; le temps de trajet dépend du trafic. Un chauffeur privé vous évite le stress des transports en commun : prise en charge à votre adresse, aide aux bagages et trajet direct jusqu'au terminal.</p>
