/**
 * Générateur de contenu SEO pour les pages VTC des communes du Val-de-Marne (94).
 * Mêmes règles : expression clé, méta 140–156, une image, H2/H3, 2500 mots min, tournures naturelles, intro personnalisée, code postal.
 */
...
import { VTC_VAL_DE_MARNE_VILLES_A } from "./vtc-val-de-marne-villes-a.js";
import { VTC_VAL_DE_MARNE_VILLES_B } from "./vtc-val-de-marne-villes-b.js";
import { VTC_VAL_DE_MARNE_VILLES_C } from "./vtc-val-de-marne-villes-c.js";
import { VTC_VAL_DE_MARNE_VILLES_F } from "./vtc-val-de-marne-villes-f.js";
import { VTC_VAL_DE_MARNE_VILLES_G } from "./vtc-val-de-marne-villes-g.js";
import { VTC_VAL_DE_MARNE_VILLES_H } from "./vtc-val-de-marne-villes-h.js";
import { VTC_VAL_DE_MARNE_VILLES_I } from "./vtc-val-de-marne-villes-i.js";
import { VTC_VAL_DE_MARNE_VILLES_J } from "./vtc-val-de-marne-villes-j.js";
import { VTC_VAL_DE_MARNE_VILLES_K } from "./vtc-val-de-marne-villes-k.js";
import { VTC_VAL_DE_MARNE_VILLES_L } from "./vtc-val-de-marne-villes-l.js";
import { VTC_VAL_DE_MARNE_VILLES_M } from "./vtc-val-de-marne-villes-m.js";
import { VTC_VAL_DE_MARNE_VILLES_N } from "./vtc-val-de-marne-villes-n.js";
import { VTC_VAL_DE_MARNE_VILLES_O } from "./vtc-val-de-marne-villes-o.js";
import { VTC_VAL_DE_MARNE_VILLES_P } from "./vtc-val-de-marne-villes-p.js";
import { VTC_VAL_DE_MARNE_VILLES_R } from "./vtc-val-de-marne-villes-r.js";
import { VTC_VAL_DE_MARNE_VILLES_S } from "./vtc-val-de-marne-villes-s.js";
import { VTC_VAL_DE_MARNE_VILLES_T } from "./vtc-val-de-marne-villes-t.js";
import { VTC_VAL_DE_MARNE_VILLES_V } from "./vtc-val-de-marne-villes-v.js";

const ALL_COMMUNES = [
  ...VTC_VAL_DE_MARNE_VILLES_A,
  ...VTC_VAL_DE_MARNE_VILLES_B,
  ...VTC_VAL_DE_MARNE_VILLES_C,
  ...VTC_VAL_DE_MARNE_VILLES_F,
  ...VTC_VAL_DE_MARNE_VILLES_G,
  ...VTC_VAL_DE_MARNE_VILLES_H,
  ...VTC_VAL_DE_MARNE_VILLES_I,
  ...VTC_VAL_DE_MARNE_VILLES_J,
  ...VTC_VAL_DE_MARNE_VILLES_K,
  ...VTC_VAL_DE_MARNE_VILLES_L,
  ...VTC_VAL_DE_MARNE_VILLES_M,
  ...VTC_VAL_DE_MARNE_VILLES_N,
  ...VTC_VAL_DE_MARNE_VILLES_O,
  ...VTC_VAL_DE_MARNE_VILLES_P,
  ...VTC_VAL_DE_MARNE_VILLES_R,
  ...VTC_VAL_DE_MARNE_VILLES_S,
  ...VTC_VAL_DE_MARNE_VILLES_T,
  ...VTC_VAL_DE_MARNE_VILLES_V,
];
...
function metaDescription(commune) {
  ...
  let text = `${kw} : chauffeur privé à ${commune.name} (${commune.cp || "94"}), transferts aéroports et gares. Réservation simple, 24h/24.`;
...
/** Gares à mentionner selon le canton (Val-de-Marne 94, Métropole du Grand Paris). */
function getGaresLabel(commune) {
  const c = (commune.canton || "").toLowerCase();
  if (c.includes("orly")) return "gares d'Orly ou de Paris";
  if (c.includes("alfortville")) return "gares d'Alfortville ou de Paris";
  if (c.includes("cachan")) return "gares de Cachan ou de Paris";
  if (c.includes("plateau") && c.includes("briard")) return "gares du Plateau briard ou de Paris";
  if (c.includes("saint-maur") && c.includes("fossés")) return "gares de Saint-Maur-des-Fossés ou de Paris";
  if (c.includes("villiers-sur-marne")) return "gares de Villiers-sur-Marne ou de Paris";
  if (c.includes("champigny")) return "gares de Champigny-sur-Marne ou de Paris";
  if (c.includes("charenton")) return "gares de Charenton-le-Pont ou de Paris";
  if (c.includes("thiais")) return "gares de Thiais ou de Paris";
  if (c.includes("choisy")) return "gares de Choisy-le-Roi ou de Paris";
  if (c.includes("créteil")) return "gares de Créteil ou de Paris";
  if (c.includes("fontenay-sous-bois")) return "gares de Fontenay-sous-Bois ou de Paris";
  if (c.includes("haÿ") || c.includes("hay")) return "gares de L'Haÿ-les-Roses ou de Paris";
  if (c.includes("kremlin")) return "gares du Kremlin-Bicêtre ou de Paris";
  if (c.includes("ivry")) return "gares d'Ivry-sur-Seine ou de Paris";
  if (c.includes("villeneuve-saint-georges")) return "gares de Villeneuve-Saint-Georges ou de Paris";
  if (c.includes("maisons-alfort")) return "gares de Maisons-Alfort ou de Paris";
  if (c.includes("nogent-sur-marne")) return "gares de Nogent-sur-Marne ou de Paris";
  if (c.includes("vincennes")) return "gares de Vincennes ou de Paris";
  if (c.includes("villejuif")) return "gares de Villejuif ou de Paris";
  if (c.includes("vitry")) return "gares de Vitry-sur-Seine ou de Paris";
  return "gares de Paris ou de Créteil";
}
...
`;
  ...
  const IMAGE_URL = "https://monchauffeurvipv3.netopsia.eu/wp-content/uploads/2026/02/vtc-val-de-marne.webp";
...
<p>La ponctualité est au cœur de notre engagement : nous suivons le trafic en temps réel et adaptons les itinéraires pour respecter vos horaires, notamment pour les transferts aéroports ou gares. Que vous habitiez ou travailliez à ${locCourt}, dans le canton de ${district} ou aux environs, nous nous déplaçons à votre adresse. De plus, nous couvrons aussi les communes voisines du Val-de-Marne.</p>
...
`;
  ...
  const sectionCouverture = getSectionCouverture({ locCourt, cp, district, gares, kw, departmentName: "Val-de-Marne" });
  const sectionQuandReserver = getSectionQuandReserver({ locCourt, cp, kw, departmentName: "Val-de-Marne" });

  const sectionPourquoi = `
<h2>Pourquoi choisir notre ${kw}</h2>
<p>Choisir notre service, c'est opter pour un transport privé de qualité, adapté aux déplacements dans le Val-de-Marne et en Île-de-France. En effet, nous nous engageons sur la ponctualité et le confort ; de plus, notre offre répond aux besoins des particuliers comme des professionnels.</p>
...
<p>Le transfert vers l'aéroport Paris-Charles de Gaulle (Roissy) depuis ${locCourt} est l'un de nos trajets les plus réservés. La distance depuis le Val-de-Marne varie selon votre commune ; le temps de trajet dépend du trafic. Un chauffeur privé vous évite le stress des transports en commun : prise en charge à votre adresse, aide aux bagages et trajet direct jusqu'au terminal.</p>
