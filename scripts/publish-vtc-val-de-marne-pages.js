/**
 * Publication des pages VTC communes Val-de-Marne (94) sur WordPress.
 * Slugs existants sous nos-secteurs, création des pages manquantes.
 *
 * Usage : node scripts/publish-vtc-val-de-marne-pages.js
 *         node scripts/publish-vtc-val-de-marne-pages.js vtc-creteil
 */

import { createPageUnderNosSecteurs, getAllPageSlugsUnderNosSecteurs } from "./wordpress-api.js";
import { getAllPagesData } from "./generate-vtc-val-de-marne-content.js";
...
if (toPublish.length === 0) {
  console.error(onlySlug ? `Aucune commune 94 avec le slug "${onlySlug}".` : "Aucune page à publier.");
  process.exit(1);
}
...
if (toCreate.length === 0) {
  console.log(`Toutes les communes 94 sont déjà publiées (${alreadyCount} page(s)). Rien à créer.`);
  process.exit(0);
}
