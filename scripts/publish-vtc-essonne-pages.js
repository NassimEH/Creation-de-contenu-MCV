/**
 * Publication des pages VTC communes Essonne (91, villes A, B, …) sur WordPress.
 * Même logique que publish-vtc-yvelines : slugs existants sous nos-secteurs, création des pages manquantes.
 *
 * Usage : node scripts/publish-vtc-essonne-pages.js
 *         node scripts/publish-vtc-essonne-pages.js vtc-arpajon
 */
...
if (toPublish.length === 0) {
  console.error(onlySlug ? `Aucune commune 91 avec le slug "${onlySlug}".` : "Aucune page à publier.");
  process.exit(1);
}
...
if (toCreate.length === 0) {
  console.log(`Toutes les communes 91 (A–B) sont déjà publiées (${alreadyCount} page(s)). Rien à créer.`);
  process.exit(0);
}
