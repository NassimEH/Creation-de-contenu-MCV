/**
 * Générateur de contenu SEO pour les pages VTC des communes de Seine-Saint-Denis (93).
...
import { VTC_SEINE_SAINT_DENIS_VILLES_A } from "./vtc-seine-saint-denis-villes-a.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_B } from "./vtc-seine-saint-denis-villes-b.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_C } from "./vtc-seine-saint-denis-villes-c.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_D } from "./vtc-seine-saint-denis-villes-d.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_E } from "./vtc-seine-saint-denis-villes-e.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_G } from "./vtc-seine-saint-denis-villes-g.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_I } from "./vtc-seine-saint-denis-villes-i.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_L } from "./vtc-seine-saint-denis-villes-l.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_M } from "./vtc-seine-saint-denis-villes-m.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_N } from "./vtc-seine-saint-denis-villes-n.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_P } from "./vtc-seine-saint-denis-villes-p.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_R } from "./vtc-seine-saint-denis-villes-r.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_S } from "./vtc-seine-saint-denis-villes-s.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_T } from "./vtc-seine-saint-denis-villes-t.js";
import { VTC_SEINE_SAINT_DENIS_VILLES_V } from "./vtc-seine-saint-denis-villes-v.js";

const ALL_COMMUNES = [
  ...VTC_SEINE_SAINT_DENIS_VILLES_A,
  ...VTC_SEINE_SAINT_DENIS_VILLES_B,
  ...VTC_SEINE_SAINT_DENIS_VILLES_C,
  ...VTC_SEINE_SAINT_DENIS_VILLES_D,
  ...VTC_SEINE_SAINT_DENIS_VILLES_E,
  ...VTC_SEINE_SAINT_DENIS_VILLES_G,
  ...VTC_SEINE_SAINT_DENIS_VILLES_I,
  ...VTC_SEINE_SAINT_DENIS_VILLES_L,
  ...VTC_SEINE_SAINT_DENIS_VILLES_M,
  ...VTC_SEINE_SAINT_DENIS_VILLES_N,
  ...VTC_SEINE_SAINT_DENIS_VILLES_P,
  ...VTC_SEINE_SAINT_DENIS_VILLES_R,
  ...VTC_SEINE_SAINT_DENIS_VILLES_S,
  ...VTC_SEINE_SAINT_DENIS_VILLES_T,
  ...VTC_SEINE_SAINT_DENIS_VILLES_V,
];
...
function metaDescription(commune) {
  ...
  let text = `${kw} : chauffeur privé à ${commune.name} (${commune.cp || "93"}), transferts aéroports et gares. Réservation simple, 24h/24.`;
...
/** Gares à mentionner selon le canton (Seine-Saint-Denis 93, Métropole du Grand Paris). */
function getGaresLabel(commune) {
  const c = (commune.canton || "").toLowerCase();
  if (c.includes("aubervilliers")) return "gares d'Aubervilliers ou de Paris";
  if (c.includes("aulnay")) return "gares d'Aulnay-sous-Bois ou de Paris";
  if (c.includes("bagnolet")) return "gares de Bagnolet ou de Paris";
  if (c.includes("blanc-mesnil")) return "gares du Blanc-Mesnil ou de Paris";
  if (c.includes("bobigny")) return "gares de Bobigny ou de Paris";
  if (c.includes("bondy")) return "gares de Bondy ou de Paris";
  if (c.includes("courneuve")) return "gares de La Courneuve ou de Paris";
  if (c.includes("drancy")) return "gares de Drancy ou de Paris";
  if (c.includes("épinay") || c.includes("epinay")) return "gares d'Épinay-sur-Seine ou de Paris";
  if (c.includes("gagny")) return "gares de Gagny ou de Paris";
  if (c.includes("noisy-le-grand")) return "gares de Noisy-le-Grand ou de Paris";
  if (c.includes("saint-ouen")) return "gares de Saint-Ouen ou de Paris";
  if (c.includes("livry")) return "gares de Livry-Gargan ou de Paris";
  if (c.includes("montreuil")) return "gares de Montreuil ou de Paris";
  if (c.includes("villemomble")) return "gares de Villemomble ou de Paris";
  if (c.includes("tremblay")) return "gares de Tremblay-en-France ou de Paris";
  if (c.includes("pantin")) return "gares de Pantin ou de Paris";
  if (c.includes("saint-denis")) return "gares de Saint-Denis ou de Paris";
  if (c.includes("sevran")) return "gares de Sevran ou de Paris";
  return "gares de Paris ou de Saint-Denis";
}
...
`;
  ...
  const IMAGE_URL = "https://monchauffeurvipv3.netopsia.eu/wp-content/uploads/2026/02/vtc-seine-saint-denis.webp";
...
<p>La ponctualité est au cœur de notre engagement : nous suivons le trafic en temps réel et adaptons les itinéraires pour respecter vos horaires, notamment pour les transferts aéroports ou gares. Que vous habitiez ou travailliez à ${locCourt}, dans le canton de ${district} ou aux environs, nous nous déplaçons à votre adresse. De plus, nous couvrons aussi les communes voisines de Seine-Saint-Denis.</p>
...
`;
  ...
  const sectionCouverture = getSectionCouverture({ locCourt, cp, district, gares, kw, departmentName: "Seine-Saint-Denis" });
  const sectionQuandReserver = getSectionQuandReserver({ locCourt, cp, kw, departmentName: "Seine-Saint-Denis" });

  const sectionPourquoi = `
<h2>Pourquoi choisir notre ${kw}</h2>
<p>Choisir notre service, c'est opter pour un transport privé de qualité, adapté aux déplacements en Seine-Saint-Denis et en Île-de-France. En effet, nous nous engageons sur la ponctualité et le confort ; de plus, notre offre répond aux besoins des particuliers comme des professionnels.</p>
...
<p>Le transfert vers l'aéroport Paris-Charles de Gaulle (Roissy) depuis ${locCourt} est l'un de nos trajets les plus réservés. La distance depuis la Seine-Saint-Denis varie selon votre commune ; le temps de trajet dépend du trafic. Un chauffeur privé vous évite le stress des transports en commun : prise en charge à votre adresse, aide aux bagages et trajet direct jusqu'au terminal.</p>
