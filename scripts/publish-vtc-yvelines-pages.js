/**
 * Publication des pages VTC communes Yvelines (78, villes A, B, …) sur WordPress.
 * Même logique que publish-vtc-communes : slugs existants sous nos-secteurs, création des pages manquantes.
 *
 * Usage : node scripts/publish-vtc-yvelines-pages.js
 *         node scripts/publish-vtc-yvelines-pages.js vtc-achères
 */
...
if (toPublish.length === 0) {
  console.error(onlySlug ? `Aucune commune 78 avec le slug "${onlySlug}".` : "Aucune page à publier.");
  process.exit(1);
}
...
if (toCreate.length === 0) {
  console.log(`Toutes les communes 78 (A–B) sont déjà publiées (${alreadyCount} page(s)). Rien à créer.`);
  process.exit(0);
}
