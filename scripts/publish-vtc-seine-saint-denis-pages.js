/**
 * Publication des pages VTC communes Seine-Saint-Denis (93) sur WordPress.
 * Slugs existants sous nos-secteurs, création des pages manquantes.
 *
 * Usage : node scripts/publish-vtc-seine-saint-denis-pages.js
 *         node scripts/publish-vtc-seine-saint-denis-pages.js vtc-aubervilliers
 */
...
if (toPublish.length === 0) {
  console.error(onlySlug ? `Aucune commune 93 avec le slug "${onlySlug}".` : "Aucune page à publier.");
  process.exit(1);
}
...
if (toCreate.length === 0) {
  console.log(`Toutes les communes 93 sont déjà publiées (${alreadyCount} page(s)). Rien à créer.`);
  process.exit(0);
}
