/**
 * Générateur de contenu SEO pour les pages VTC des communes (Seine-et-Marne 77, toutes les communes).
 * Mêmes règles : expression clé, méta 140–156, une image, H2/H3, 2500 mots min, tournures naturelles.
 */
...
import { VTC_SEINE_ET_MARNE_VILLES_D } from "./vtc-seine-et-marne-villes-d.js";
import { VTC_SEINE_ET_MARNE_VILLES_E } from "./vtc-seine-et-marne-villes-e.js";
import { VTC_SEINE_ET_MARNE_VILLES_F } from "./vtc-seine-et-marne-villes-f.js";
import { VTC_SEINE_ET_MARNE_VILLES_G } from "./vtc-seine-et-marne-villes-g.js";
import { VTC_SEINE_ET_MARNE_VILLES_H } from "./vtc-seine-et-marne-villes-h.js";
import { VTC_SEINE_ET_MARNE_VILLES_I } from "./vtc-seine-et-marne-villes-i.js";
import { VTC_SEINE_ET_MARNE_VILLES_J } from "./vtc-seine-et-marne-villes-j.js";
import { VTC_SEINE_ET_MARNE_VILLES_L } from "./vtc-seine-et-marne-villes-l.js";
import { VTC_SEINE_ET_MARNE_VILLES_M } from "./vtc-seine-et-marne-villes-m.js";
import { VTC_SEINE_ET_MARNE_VILLES_N } from "./vtc-seine-et-marne-villes-n.js";
import { VTC_SEINE_ET_MARNE_VILLES_O } from "./vtc-seine-et-marne-villes-o.js";
import { VTC_SEINE_ET_MARNE_VILLES_P } from "./vtc-seine-et-marne-villes-p.js";
import { VTC_SEINE_ET_MARNE_VILLES_Q } from "./vtc-seine-et-marne-villes-q.js";
import { VTC_SEINE_ET_MARNE_VILLES_R } from "./vtc-seine-et-marne-villes-r.js";
import { VTC_SEINE_ET_MARNE_VILLES_S } from "./vtc-seine-et-marne-villes-s.js";
import { VTC_SEINE_ET_MARNE_VILLES_T } from "./vtc-seine-et-marne-villes-t.js";
import { VTC_SEINE_ET_MARNE_VILLES_U } from "./vtc-seine-et-marne-villes-u.js";
import { VTC_SEINE_ET_MARNE_VILLES_V } from "./vtc-seine-et-marne-villes-v.js";
import { VTC_SEINE_ET_MARNE_VILLES_Y } from "./vtc-seine-et-marne-villes-y.js";

const ALL_COMMUNES = [
  ...VTC_SEINE_ET_MARNE_VILLES_A,
  ...VTC_SEINE_ET_MARNE_VILLES_B,
  ...VTC_SEINE_ET_MARNE_VILLES_C,
  ...VTC_SEINE_ET_MARNE_VILLES_D,
  ...VTC_SEINE_ET_MARNE_VILLES_E,
  ...VTC_SEINE_ET_MARNE_VILLES_F,
  ...VTC_SEINE_ET_MARNE_VILLES_G,
  ...VTC_SEINE_ET_MARNE_VILLES_H,
  ...VTC_SEINE_ET_MARNE_VILLES_I,
  ...VTC_SEINE_ET_MARNE_VILLES_J,
  ...VTC_SEINE_ET_MARNE_VILLES_L,
  ...VTC_SEINE_ET_MARNE_VILLES_M,
  ...VTC_SEINE_ET_MARNE_VILLES_N,
  ...VTC_SEINE_ET_MARNE_VILLES_O,
  ...VTC_SEINE_ET_MARNE_VILLES_P,
  ...VTC_SEINE_ET_MARNE_VILLES_Q,
  ...VTC_SEINE_ET_MARNE_VILLES_R,
  ...VTC_SEINE_ET_MARNE_VILLES_S,
  ...VTC_SEINE_ET_MARNE_VILLES_T,
  ...VTC_SEINE_ET_MARNE_VILLES_U,
...
function metaDescription(commune) {
  ...
  let text = `${kw} : chauffeur privé à ${commune.name} (${commune.cp || "77"}), transferts aéroports et gares. Réservation simple, 24h/24.`;
  if (text.length < 140) text += " Réservez en quelques clics.";
  if (text.length > 156) text = text.slice(0, 153) + "...";
  return text;
}

/** Retourne 2 autres communes (pour liens internes), sans la commune courante. */
function otherCommunesForLinks(commune) {
  return ALL_COMMUNES.filter((c) => c.slug !== commune.slug).slice(0, 2);
}

/** Gares à mentionner selon le canton / l’interco (SEO local cohérent). */
function getGaresLabel(commune) {
  const c = (commune.canton || "").toLowerCase();
  const i = (commune.interco || "").toLowerCase();
  if (c.includes("meaux") || i.includes("meaux")) return "gares de Meaux ou de Paris";
  if (c.includes("melun") || i.includes("melun")) return "gares de Melun ou de Paris";
  if (c.includes("fontainebleau") || i.includes("fontainebleau")) return "gares de Fontainebleau ou de Melun";
  if (c.includes("provins") || i.includes("provins")) return "gares de Provins ou de Paris";
  if (c.includes("montereau") || i.includes("montereau")) return "gares de Montereau ou de Melun";
  if (c.includes("nemours") || i.includes("nemours")) return "gares de Nemours ou de Melun";
  if (c.includes("coulommiers") || i.includes("coulommiers")) return "gares de Coulommiers ou de Paris";
  if (c.includes("lagny") || i.includes("marne et gondoire") || i.includes("vallée de la marne")) return "gares de Lagny ou de Paris";
  if (c.includes("savigny") || i.includes("sénart")) return "gares de Savigny ou de Melun";
  if (c.includes("villeparisis") || i.includes("roissy")) return "gares de Villeparisis ou de Paris";
  if (c.includes("mitry") || i.includes("roissy")) return "gares de Mitry ou de Paris";
  if (c.includes("serris") || i.includes("val d'europe")) return "gares de Marne-la-Vallée ou de Paris";
  if (c.includes("ozoir") || i.includes("ferrière")) return "gares d'Ozoir ou de Paris";
  if (c.includes("torcy") || i.includes("vallée de la marne")) return "gares de Torcy ou de Paris";
  if (c.includes("nangis")) return "gares de Nangis ou de Melun";
  return "gares de Paris ou de Melun";
}

/** Formulation d’ancrage géographique pour l’intro (évite « Environs », « Centre du village » partout). */
...
`;
  ...
  const sectionCouverture = getSectionCouverture({ locCourt, cp, district, gares, kw, departmentName: "Seine-et-Marne" });
  const sectionQuandReserver = getSectionQuandReserver({ locCourt, cp, kw, departmentName: "Seine-et-Marne" });

  const sectionPourquoi = `
<h2>Pourquoi choisir notre ${kw}</h2>
<p>Choisir notre service, c'est opter pour un transport privé de qualité, adapté aux déplacements en Seine-et-Marne et en Île-de-France. En effet, nous nous engageons sur la ponctualité et le confort ; de plus, notre offre répond aux besoins des particuliers comme des professionnels.</p>
...
<p>Le transfert vers l'aéroport Paris-Charles de Gaulle (Roissy) depuis ${locCourt} est l'un de nos trajets les plus réservés. La distance depuis la Seine-et-Marne varie selon votre commune ; le temps de trajet dépend du trafic. Un chauffeur privé vous évite le stress des transports en commun : prise en charge à votre adresse, aide aux bagages et trajet direct jusqu'au terminal.</p>
