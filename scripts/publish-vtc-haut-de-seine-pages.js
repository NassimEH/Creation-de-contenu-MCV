/**
 * Publication des pages VTC communes Hauts-de-Seine (92) sur WordPress.
 * Slugs existants sous nos-secteurs, création des pages manquantes.
 *
 * Usage : node scripts/publish-vtc-haut-de-seine-pages.js
 *         node scripts/publish-vtc-haut-de-seine-pages.js vtc-antony
 */
...
if (toPublish.length === 0) {
  console.error(onlySlug ? `Aucune commune 92 avec le slug "${onlySlug}".` : "Aucune page à publier.");
  process.exit(1);
}
...
if (toCreate.length === 0) {
  console.log(`Toutes les communes 92 sont déjà publiées (${alreadyCount} page(s)). Rien à créer.`);
  process.exit(0);
}
