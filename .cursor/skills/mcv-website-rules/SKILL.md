...
- **Métropoles** : Lyon, Marseille, Toulouse, Bordeaux, Lille, Nice, Nantes, Strasbourg, Montpellier, Rennes, Grenoble, Tours, **Orléans**. Données dans `vtc-metropoles-data.js`, contenu dans `generate-vtc-metropoles-content.js` (2500+ mots, SEO, alternative taxi). Slugs : `vtc-lyon`, `vtc-marseille`, `vtc-tours`, `vtc-orleans`, etc. Publication : `publish-vtc-metropoles-pages.js`, mise à jour : `update-vtc-metropoles-pages.js`.
- **Fontainebleau** : page commune 77 avec contenu personnalisé (fusion texte existant + SEO). Fichier dédié `generate-vtc-fontainebleau-content.js` (téléphone 06 18 33 21 74, tarifs Orly 119€ / CDG 159€, INSEAD, Aigle Noir, Victoria, Grand Parquet). Branché dans `getPageData()` de `generate-vtc-communes-content.js` pour la commune « Fontainebleau ». Slug : `vtc-fontainebleau`.
- **Sénart** : page secteur 77 (agglomération, pas une commune). Contenu dédié dans `generate-vtc-senart-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, villes : Lieusaint, Moissy-Cramayel, Combs-la-Ville, Réau, Nandy, Saint-Pierre-du-Perray, Tigery, Savigny-le-Temple, Cesson, Vert-Saint-Denis, Carré-Sénart Lieusaint). Entrée `SENART_ENTRY` dans `generate-vtc-communes-content.js`, branchée dans `getPageData()` et incluse dans `ALL_PAGES_77`. Slug : `vtc-senart`.
- **Moissy-Cramayel** : page commune 77 avec contenu personnalisé. Fichier dédié `generate-vtc-moissy-cramayel-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, 77550, navette aéroport sur Sénart, lien Mairie de Moissy-Cramayel). Branché dans `getPageData()` de `generate-vtc-communes-content.js` pour la commune « Moissy-Cramayel ». Slug : `vtc-moissy-cramayel`.
- **Lieusaint** : page commune 77 avec contenu personnalisé. Fichier dédié `generate-vtc-lieusaint-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, 77127, offre VTC CARRÉ SÉNART au Westfield Carré Sénart, théâtre de Lieusaint, lien ville Lieusaint). Branché dans `getPageData()` pour la commune « Lieusaint ». Slug : `vtc-lieusaint`.
- **Savigny-le-Temple** : page commune 77 avec contenu personnalisé. Fichier dédié `generate-vtc-savigny-le-temple-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, 77176, navette aéroport sur Sénart, lien ville Savigny-le-Temple). Branché dans `getPageData()` pour la commune « Savigny-le-Temple ». Slug : `vtc-savigny-le-temple`.
- **Combs-la-Ville** : page commune 77 avec contenu personnalisé. Fichier dédié `generate-vtc-combs-la-ville-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, 77380, navette aéroport sur Sénart, lien commune Combs-la-Ville). Branché dans `getPageData()` pour la commune « Combs-la-Ville ». Slug : `vtc-combs-la-ville`.
- **Melun** : page commune 77 avec contenu personnalisé. Fichier dédié `generate-vtc-melun-content.js` (téléphone 06 18 33 21 74, CHAUFF'HEURE VIP, 77000, alternative taxi depuis 2015, confirmation mail/SMS, lien ville Melun). Branché dans `getPageData()` pour la commune « Melun ». Slug : `vtc-melun`.
...
- **Pages hub** (maillage interne et mots-clés) : « Chauffeur privé aéroport » (slug `chauffeur-prive-aeroport`), « Chauffeur privé gares » (`chauffeur-prive-gares`), « Chauffeur privé Île-de-France » (`chauffeur-prive-ile-de-france`), « Alternative grève Seine-et-Marne » (`alternative-greve-seine-et-marne`), « Tourisme Seine-et-Marne en VTC » (`tourisme-seine-et-marne`), « VTC Théâtre-Sénart Lieusaint » (`vtc-theatre-senart-lieusaint`), « VTC Carré-Sénart » (`vtc-carre-senart`), « Transport VTC Longues distances » (`vtc-longues-distances`), « Transport par Taxi pour un long trajet » (`taxi-long-trajet`). Contenu dans `generate-hub-aeroport-content.js`, `generate-hub-gares-content.js`, `generate-hub-ile-de-france-content.js`, `generate-alternative-greve-77-content.js`, `generate-tourisme-seine-et-marne-content.js`, `generate-theatre-senart-content.js`, `generate-carre-senart-content.js`, `generate-vtc-longues-distances-content.js`, `generate-taxi-long-trajet-content.js`. Publication et mise à jour : `publish-hub-pages.js` (crée si absent, met à jour sinon).
